...
# --- Funktioner för Aktiedata med utökad DEBUG ---
...
def get_stock_metrics(ticker_symbol: str):
    ...
    try:
        ...
        # Skriv ut rådata från varje modul för att se vad vi får
        ...
        if not price_module_data and not summary_detail_module_data and not key_stats_module_data and not summary_profile_module_data and not financial_data_module_data:
             st.warning(f"[Debug get_stock_metrics] Ingen data alls i modulerna för {ticker_symbol}")
...
def search_for_tickers_by_name(company_query: str) -> list:
    ...
    try:
        ...
        for i, quote in enumerate(quotes_found):
            ...
            # Kontrollera att det är en dict och har nödvändiga nycklar
            if isinstance(quote, dict) and 'symbol' in quote and \
               (quote.get('shortname') or quote.get('longname')): # Måste ha antingen kort eller långt namn
                # TEMPORÄRT: Ta bort quoteType-filter för att se allt som "amazon" ger
                ...
                valid_results.append(quote) # Lägg till allt för nu för att se vad vi får
...
with col2_lottie:
    ...
    except requests.exceptions.JSONDecodeError as e_json_lottie: # Unikt namn för variabel
...
with st.sidebar:
    st.header("🔍 Sök Aktie")
    # Använd en unik nyckel för sökinput för att kunna återställa den om så önskas
    ...
    if 'selected_ticker_for_metrics' not in st.session_state:
        st.session_state.selected_ticker_for_metrics = "VOLV-B.ST" # Default
    ...
    if company_name_search_query:
        ...
        if found_tickers:
            ...
            for item in found_tickers:
                ...
                display_str = f"{name} ({symbol}) - {exchange_display} [{quote_type_display}]" # Lägg till quoteType
                ticker_options_display[display_str] = symbol
            
            def on_ticker_select_sidebar(): # Callback för selectbox
                selected_key = st.session_state.search_result_selectbox_sidebar_key
                if selected_key and ticker_options_display[selected_key] != "":
                    st.session_state.selected_ticker_for_metrics = ticker_options_display[selected_key]
                    # Återställ sökfältet för att "rensa" sökningen och selectboxen
                    st.session_state.company_search_input_field = "" 
            ...
            st.selectbox(
                ...
                index=0,
                on_change=on_ticker_select_sidebar
            )
...
# (Som tidigare, med tabs för analysmetoder)
...
with input_col2:
    manual_text_input = st.text_area("✏️ Eller klistra in text manuellt här (valfritt):", height=205, key="manual_text_input_area") # Unik nyckel
...
with tab_rag_analysis:
    if "user_question_rag_tab" not in st.session_state: # Unik nyckel för session state
    ...
    if st.button("Starta frågebaserad analys", key="btn_rag_analysis_tab_main", use_container_width=True):
        if text_to_analyze and len(text_to_analyze.strip()) > 20:
            with st.spinner("🤖 GPT söker och analyserar baserat på din fråga..."):
                source_id = (html_link or (uploaded_file.name if uploaded_file else text_to_analyze[:50])) + "_embeddings_v5" # Ny version för cache
                ...
                if stock_metrics_data and "error" not in stock_metrics_data: # Använd stock_metrics_data från sidebar
                    beta_val = stock_metrics_data.get('Beta')
                    pe_val = stock_metrics_data.get('PE')
